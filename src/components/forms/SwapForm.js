...
export default function SwapForm(props) {
  ...
  async function handleSwapChange(amount, from, to){
    if((from.name == "ETH" && amount > 0.01) || (from.name == "USDC" && amount > 100) || amount > 1115 || amount < 0) return;
  ...
  async function handleReceiveChange(amount, from, to){
    if((to.name == "ETH" && amount > 0.01) || (to.name == "USDC" && amount > 100) || amount > 1115 || amount < 0) return;
